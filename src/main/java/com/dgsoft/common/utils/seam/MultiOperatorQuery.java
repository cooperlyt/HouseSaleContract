 ...
 *
 */
public abstract class MultiOperatorQuery<T, E>
        extends PersistenceController<T>
{
   ...
   // private List<Expressions.ValueExpression> restrictionParameters;
    ...
    {
        if (parsedEjbql==null || (restrictionGroup != null && !restrictionGroup.isParsed()))
        {
            ...
            if (restrictionGroup != null){
                restrictionGroup.parseEjbql(queryParameters.size());
            }
    ...
    {
        setQueryParameterValues( RestrictionGroup.getParameterValues(getQueryParameters()) );
        if (restrictionGroup != null) {
            restrictionGroup.evaluateAllParameters();
        }
    }

    protected boolean isAnyParameterDirty()
    {
        return RestrictionGroup.isAnyParameterDirty(getQueryParameters(), getQueryParameterValues())
                || (restrictionGroup != null && restrictionGroup.isAnyParameterDirty());
    }
